...
import com.thoughtworks.wechat.utils.ReflectionUtils;

import java.lang.reflect.Field;

/**
 * Created by liyang on 2015/3/14.
 */
public class ScaleCalculator {
    ...
    public static ScaleCalculator getInstance() {
        if (mInstance == null) {
            return init(WeChatApp.sContext);
        }
        return mInstance;
    }
    ...
    public final int scaleWidth(int pixel) {
        return Math.round(this.adpaterDensity(pixel) * mCurrentWidth / BASE_WIDTH);
    }

    public final int scaleHeight(int pixel) {
        return Math.round(this.adpaterDensity(pixel) * mCurrentHeight / BASE_HEIGHT);
    }
